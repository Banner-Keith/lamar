...
public class ServiceFamily
{
    ...
    public ServiceFamily(Type serviceType, IDecoratorPolicy[] decoratorPolicies, params Instance[] instances)
    {
        ...
        foreach (var instance in instances) _instances.Add(instance.Name, instance);
    ...
    {
        foreach (var decoratorPolicy in decoratorPolicies) yield return decoratorPolicy;
    ...
    public AppendState Append(Instance[] instances, IDecoratorPolicy[] decoration)
    {
        ...
        foreach (var instance in instances) instance.IsDefault = false;
        ...
        foreach (var instance in all) instance.IsOnlyOneOfServiceType = false;
        ...
        if (currentDefault == null && Default != null)
        {
            return AppendState.NewDefault;
        }
    ...
    public Instance InstanceFor(string name)
    {
        return Instances.ContainsKey(name)
            ? Instances[name]
            : _instances.Values.ToArray().FirstOrDefault(x => x.Name == name);
    }
